...
\todo[inline]{define the diameter of a graph}
\todo[inline]{define the degree (in or out) of a graph}
...
Between each node pair not the shortest path is calculated but a set of 10 different paths. On each of those paths 3 payments of different sizes are tried to be executed. A \emph{1-satoshi payment}, a \emph{micro payment} of 10'000 satoshi and a \emph{normal payment} of 100'000 satoshi. Each node pair then receives a score between 1 and 10 per payment size depending of how many paths could route the payment. The median of all node pairs then gives an indication about how successful different payment sizes are considering multiple paths in sequence. 
...
The \emph{degree centrality} is an easy to calculate measure. It simply counts the degree - the number of edges connected to a node. The higher the value, the more central a node is \citep{golbeck_analyzing_2013}. \todo{make reference to the formal definition of degree}
...
A similar picture can be drawn from the \emph{success rate} measure. Since this is the ratio of all node pairs able to rout at least 1 satoshi there are no spikes. The improving effects are more distributed among the different levels of participation and start with 20\% and reach up to 80\% of all node pairs which can send a payment.
